...
\\\\textbf{References:} Custom CTF construction; related to Nitaj's constrained prime analysis\`,
  usageGuide: 'This attack factors n when q is derived from p through a modular relationship: q·e ≡ 1 (mod p).\\n\\nHow to use:\\n1. You have n and e, and know that q is computed as q = e^(-1) mod p\\n2. Provide n and e\\n3. The attack solves the equation k*p^2 + p - n*e = 0 to recover p\\n\\nTip: This is a constructed pattern (CTF-style weak keygen where q is derived from p); treat any real-world occurrence as broken key generation, not as a supported construction.',
  priority: 'medium',
  applicableCheck: rsaNeeds.nE,
};

export const generateTestcase = (): Record<string, string> => {
  // Small-case sizing (p ~96-bit, n ~192-bit): the generator rejection-samples
  // q = e^-1 mod p for primality (~1/66 hit rate), which is minutes at 512-bit
  // primes and milliseconds here. The k-iteration attack itself is bit-size
  // agnostic (k ~= e for the testcase regardless of size).
  const e = 65537n;
