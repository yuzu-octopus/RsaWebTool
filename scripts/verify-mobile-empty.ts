...
async function main() {
  ...
  try {
    ...
    await page.getByRole('button', { name: 'Choose an attack' }).click();
    await page.locator('#sidebar-attack-pollard-rho').waitFor({ timeout: 5000 });
    const text = await page.locator('body').innerText();
    assert(!text.includes(FALLBACK_TEXT), 'Mobile drawer navigation crashed the workspace');
    console.log('PASS: mobile empty state with drawer entry');
  } finally {
    await browser.close();
    server.kill();
  }
