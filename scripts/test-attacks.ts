...
const L2B_SEMANTIC_VALIDATORS: Record<string, SemanticValidator> = {
  ...
  'multi-prime-gcd': (vals) => {
    if (!vals.moduli_list) return { status: 'fail', detail: 'Missing moduli_list' };
    const lines = vals.moduli_list.split('\n').filter((s) => s.trim());
  ...
  'franklin-reiter': (vals) => {
    if (!vals.a || !vals.b) return { status: 'fail', detail: 'Missing a or b' };
    const a = BigInt(vals.a);
    const b = BigInt(vals.b);
    if (a !== 1n || b !== 12345n) return { status: 'fail', detail: `Expected a=1, b=12345, got a=${a}, b=${b}` };
    return { status: 'pass', detail: 'a=1, b=12345 for linear relationship' };
  },
  ...
  'related-message': (vals) => {
    ...
    const a = BigInt(vals.a);
    const b = BigInt(vals.b);
    if (a !== 2n || b !== 3n) return { status: 'fail', detail: `Expected a=2, b=3, got a=${a}, b=${b}` };
    return { status: 'pass', detail: 'a=2, b=3 for linear relationship' };
  },
  ...
  'coppersmith-short-pad': (vals) => {
    ...
    if (nBits > 130) return { status: 'fail', detail: `n is ${nBits} bits (> 130 may timeout in SageCell)` };
  ...
  'partial-pq-bits': (vals) => {
    ...
    if (vals.bitPosition !== 'msb') return { status: 'fail', detail: `Expected bitPosition=msb, got ${vals.bitPosition}` };
